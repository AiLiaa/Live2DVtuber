...
public class HiyoriController : MonoBehaviour, ISaveable
{
    ...
    void ArmMove()
    {
        ...
        parameter.Value = -Mathf.Clamp(shoudler_r_rotaton / 9 - 20, -10, 30);

        parameter = model.Parameters[62];
        parameter.Value = Mathf.Clamp(elbow_r_rotaton /7.2f-40, -40, 10);

        parameter = model.Parameters[63];
        parameter.Value = Mathf.Clamp(wrist_r_rotaton / 6 - 30, -30, 30);

        parameter = model.Parameters[65];
        parameter.Value = -Mathf.Clamp(shoudler_l_rotaton / 9 - 20, -10, 30);

        parameter = model.Parameters[66];
        parameter.Value = Mathf.Clamp(elbow_l_rotaton / 7.2f - 40, -40, 10);

        parameter = model.Parameters[67];
        parameter.Value = Mathf.Clamp(wrist_l_rotaton / 6 - 30, -30, 30);
    }
    ...
    public void PopulateSaveData(NatoriPref a_SaveData)
    {
        throw new NotImplementedException();
    }

    public void LoadFromSaveData(NatoriPref a_SaveData)
    {
        throw new NotImplementedException();
    }
}
